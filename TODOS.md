...
- Voice: read-aloud (TTS) + dictation (STT): [`SPECs/voice-read-aloud-dictation-spec.md`](SPECs/voice-read-aloud-dictation-spec.md) — macOS-only, offline. Read-aloud uses the WebView's Web Speech API (system offline voices) with word highlight + mini-player (`Cmd+Shift+R`); dictation runs Whisper locally (whisper-rs, Metal, auto-downloaded model) streaming at the cursor (`Cmd+Shift+D`). Worksheet: [`SPECs/Agent/worksheet-voice-read-aloud-dictation.md`](SPECs/Agent/worksheet-voice-read-aloud-dictation.md).
