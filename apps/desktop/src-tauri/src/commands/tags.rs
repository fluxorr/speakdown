// Rust implementation for tag extraction
use crate::{AppError, tags::TagIndex};

/// Extract tags from a markdown file content
fn extract_tags_from_content(content: &str, path: &str) -> Result<Vec<String>, AppError> {
...
pub async fn get_tags_from_content(path: String, content: String) -> Result<Vec<String>, AppError> {
    let tags = extract_tags_from_content(&content, &path)?;
    Ok(tags)
}
