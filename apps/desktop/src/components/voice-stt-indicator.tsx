...
export function VoiceSttIndicator() {
  ...
  return (
    <AnimatePresence mode="wait">
      {modelError ? (
        <motion.div key="model-error" {...slideUp} transition={spring}>
        ...
        <motion.div key="downloading" {...slideUp} transition={spring}>
        ...
        <motion.div key="error" {...slideUp} transition={spring}>
          <div className="fixed bottom-4 left-1/2 z-50 -translate-x-1/2 rounded-lg border border-red-500/40 bg-background/95 px-3 py-2 text-xs text-red-400 shadow-lg backdrop-blur">
            Dictation error: {error}
          </div>
        </motion.div>
      ) : phase === "starting" ? (
        <motion.div key="starting" {...slideUp} transition={spring}>
        ...
        <motion.div key="listening" {...slideUp} transition={spring}>
